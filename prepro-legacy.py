...
def _process_caption_data(caption_file, image_dir, max_length):
  ...
  # data is a list of dictionaries which contains 'captions', 'file_name' and 'image_id' as key.
  data = []
  for ann in caption_data['annotations']:
    ann['file_name'] = os.path.join(image_dir, id_to_filename[ann['image_id']])
    data.append(ann)

  # convert to pandas dataframe (for later visualization or debugging)
  caption_data = pd.DataFrame.from_dict(data)

  # Don't care about caption ids
  del caption_data['id']

  caption_data.sort_values(by='image_id', inplace=True)
  caption_data = caption_data.reset_index(drop=True)

  # Reformat captions and find indexes of those we don't want
  del_idx = []
  for i, caption in enumerate(caption_data['caption']):
    # TODO: Break this into 3 regexes
    caption = caption.replace('.', '').replace(',', '').replace("'", "").replace('"', '')
    caption = caption.replace('&', 'and').replace('(', '').replace(")", "").replace('-', ' ')
    caption = " ".join(caption.split())  # replace multiple spaces

    # lowercase all the captions
  ...
  # caption_data is a pandas Dataframe with headers: [caption, image_id, file_name]
...
def main():
  ...
  with tf.Session() as sess:
    ...
    for split in ['train', 'val', 'test']:
      ...
      start_intv = range(0, n_examples, batch_size)
      end_intv = range(batch_size, n_examples + batch_size, batch_size)

      for start, end in zip(start_intv, end_intv):
        ...
        print ("Processed %d %sd features.." % (end, split))
